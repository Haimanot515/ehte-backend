export enum AuditEventEnum {
  ...
  // that service was calling eventEmitter.emit('victim_profile.claimed', ...)
  // etc. with lowercase, dot-separated literal strings instead of this
  // enum, so EventEmitter2's exact-string matching meant AuditLogListener
  // (which listens for the ALL_CAPS enum values above) never fired for
  // any of these 12 actions. These give each of those actions its own
  // first-class enum member so the fix can route through emitAudit()
  // like every other service instead of emitting untyped strings.
